...
config_file = File.join(ANSIBLE_PATH, 'group_vars', 'development', 'grav_sites.yml')
...
if File.exists?(config_file)
  grav_sites = YAML.load_file(config_file)['grav_sites']
  fail_with_message "No sites found in #{config_file}." if grav_sites.to_h.empty?
else
...
Vagrant.configure('2') do |config|
  ...
  hostname, *aliases = grav_sites.flat_map { |(_name, site)| site['site_hosts'] }
  ...
  if Vagrant::Util::Platform.windows? and !Vagrant.has_plugin? 'vagrant-winnfsd'
    grav_sites.each_pair do |name, site|
